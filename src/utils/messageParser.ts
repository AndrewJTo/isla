import {FullName, Message} from '../types/message';
...
export function parseMessage(text: string): Message {
  ...
  if (!nameComponents || nameComponents.length !== 2) {
    throw new MessageParseError('Could not parse full name');
  }
  ...
  if (names.length < 2) {
    throw new MessageParseError(
      'At least a first name and surname must be provided'
    );
  }
  ...
  if (
    ...
    throw new MessageParseError('Could not parse date of birth');
  }
  ...
  if (!conditionComponents || conditionComponents.length !== 2) {
    throw new MessageParseError('Could not parse condition');
  }
